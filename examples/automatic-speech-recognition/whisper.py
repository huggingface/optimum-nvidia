...
if __name__ == "__main__":
    ...
    model = WhisperForConditionalGeneration.from_pretrained(args.model, torch_dtype=torch_dtype)
    ...
    processor = AutoProcessor.from_pretrained(args.model)  # TODO: save processor in args.output (like ORTModel)

    data = load_dataset("hf-internal-testing/librispeech_asr_dummy", "clean", split="validation")

    inputs = processor(data[0]["audio"]["array"], return_tensors="pt", sampling_rate=data[0]["audio"]["sampling_rate"]).to("cuda")
