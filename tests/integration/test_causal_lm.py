...
def test_pipeline(model_type: str):
    ...
    for model_id in model_ids:
        ...
        pipe_torch = transformers_pipeline(task="text-generation", model=model_id, device="cuda", torch_dtype=torch.float16)

        with torch.no_grad():
            res_torch = pipe_torch("Today I am in Paris and I would like to eat crepes.", add_special_tokens=True, max_new_tokens=20, **kwargs)
        ...
        with torch.no_grad():
            res_trt = pipe_trt("Today I am in Paris and I would like to eat crepes.", max_new_tokens=20, **kwargs)
