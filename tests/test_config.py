...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            ...
            (
                QuantMode.from_description(
                    quantize_weights=True, per_group=True, use_int4_weights=False
                ),
                "W8A16_AWQ",
            ),
        ),
        (
            ...
            (
                QuantMode.from_description(
                    quantize_weights=True, per_group=False, use_int4_weights=True
                ),
                "W4A16_GPTQ",
            ),
        ),
...
def test_convert_quantization_config_to_trt(
    ...
    assert (
        convert_quant_method_to_trt(method, weight_num_bits, activation_num_bits)
        == expected
    )
