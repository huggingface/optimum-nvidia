...
There is currently no `pip` support for `optimum-nvidia` as transitive dependencies are missing on the PyPi index.
The easiest ways to get started are either:
- Pulling prebuilt docker container `huggingface/optimum-nvidia`
...
```bash
cd third-party/tensorrt-llm && make -C docker release_build CUDA_ARCHS="<TARGET_SM>-real"
```
