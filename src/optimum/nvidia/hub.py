...
import torch
from huggingface_hub import ModelHubMixin, cached_assets_path, snapshot_download
...
class HuggingFaceHubModel(ModelHubMixin):
    @classmethod
    ...
    def _from_pretrained(
        ...
        **model_kwargs,
        ...
        mapping = get_sharding_info(**model_kwargs)
        ...
        if len(engines_folders) == 0:
            ...
            with TensorRTModelConverter("out") as converter:
                checkpoints = converter.convert(local_path, config, mapping)
                engines = converter.build()
