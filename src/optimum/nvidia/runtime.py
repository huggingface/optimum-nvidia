...
class TRTEngineForCausalLM(TRTEnginePretrainedModel):
    __slots__ = ("_config", "_mapping", "_session", "_cache", "_use_packed_inputs", "_max_beam_width")
    ...
    def generate(self,
        inputs: Union[PackedTensor, torch.Tensor],
        ...
        with torch.no_grad():
            if isinstance(inputs, torch.Tensor):
                ...
                if attention_mask is not None:
                    lengths = attention_mask.sum(dim=0)
                elif input_ids.ndim == 1:
                    lengths = torch.tensor([inputs.size(0)], dtype=torch.int32)
                elif input_ids.ndim == 2 and inputs.size(0) == 1:
                ...
                input_ids = torch.nested.nested_tensor(inputs)
                lengths = torch.tensor([tensor.size(0) for tensor in inputs], dtype=torch.int32)
            ...
            trt_outputs = ctrrt.GenerationOutput(ids=torch.empty(10000, dtype=torch.int32), lengths=torch.empty(10000, dtype=torch.int32))
